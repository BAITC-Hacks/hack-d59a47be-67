...
**Текущий статус:** реализованы и локально проверены backend/API/SQLite/авторизация, детерминированные навыки и атомарный импорт. Запуск: `scripts/dev`, проверки: `scripts/test -q`. Актуальные команды и ограничения — [HANDOFF](docs/HANDOFF.md). AI-ядро и frontend имеют отдельных владельцев; готовность модели не заявляется.
...
**Первый будущий проверяемый результат:** основной сценарий «профиль → объяснимая рекомендация → завершение активности → обновлённый прогресс» целиком проходит на согласованном синтетическом ответе; затем те же экраны подключаются к настоящему API.

**Границы:** backend рассчитывает навыки, доступность мероприятий и прогресс, проверяет права; AI-модуль выбирает мероприятия из допустимых кандидатов; frontend отображает серверные данные и отправляет команды. Стыки и порядок работы описаны в [API-контракте](docs/api-contract.md) и [плане команды](docs/team-plan.md).

**Текущий статус:** Подготовлена документация зоны Frontend / UX. Реализация интерфейса ещё не начата.
