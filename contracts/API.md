...
| `GET /api/hr/analytics` | Только HR | `window_days` 1–365, по умолчанию 90 | `HRAnalyticsResponse` |
...
Учётные записи, role и employee_id назначает сервер. В `LoginRequest` допустимы только `username` и `password`; регистрации рабочих аккаунтов через API нет. Клиент использует cookie `cq_session` с `credentials: "include"`. Токен CSRF берётся из ответа login `SessionResponse.csrf_token` и передаётся в `X-CSRF-Token`; cookie — HttpOnly, срок ограничен; HTTPS-staging использует Secure. Backend хранит только hash CSRF. Для сохранения CSRF после перезагрузки страницы frontend может использовать sessionStorage; иначе нужен повторный login. `/api/me` возвращает только UserIdentity и не восстанавливает CSRF. Команды создания учётных записей находятся в `docs/HANDOFF.md`.

Origin должен совпадать с явно разрешённым origin целиком (scheme, host, port). В частном режиме клиент не передаёт роль или идентификатор владельца сессии. Связь сотрудника определяется сохранённой учётной записью. Чужой employee id не даёт сотруднику доступ даже при корректном теле запроса.
...
`GET /api/hr/analytics?window_days=90` возвращает один согласованный срез revision: `skill_gaps`, `attention`, `participation` и пояснения `notes`. Разрыв считается относительно разрешённой карьерной цели. Для каждого навыка знаменатель — число сотрудников, чья цель требует этот навык; средний разрыв — среди сотрудников с недостаточным уровнем. Основания `no_history`, `no_recent_completion`, `repeated_no_show`, `no_candidates`, `no_target` опираются на проверяемые записи и правила, а не предположение о мотивации. Период заканчивается датой набора, обе границы включены. Симуляции исключены из участия; приблизительные даты истории и влияние симуляций на текущие навыки отражены в notes. Доступ только HR; другие посетители публичного сайта не входят в этот срез.

Если есть цель, разрывы и полезные доступные активности, `attention` дополнительно проверяет **последний сохранённый результат подбора**. Профили, revision и результаты читаются в одной read transaction, AI не вызывается. `recommendation_missing` — результата ещё нет; `recommendation_stale` — его revision устарела после изменения цели, завершения или импорта; `ai_not_configured` — последний актуальный ответ `not_configured`; `recommendation_unavailable` — актуальный `unavailable` либо ответ без валидной непустой рекомендации. Последний неудачный подбор не скрывается за более ранним успешным. Актуальный `ok` с карточками не создаёт такой сигнал. Достигнутая цель не считается отсутствием следующего шага. Отсутствие кандидатов по-прежнему отдельно обозначается `no_candidates`, отсутствие цели — `no_target`. Эти сигналы не ограничены периодом истории: они описывают текущий следующий шаг.

## Изолированная публичная демонстрация

Включается отдельно `PUBLIC_DEMO_MODE=true` на пустой служебной БД. Сервер откажется запускать этот режим поверх рабочей базы с каталогом или аккаунтами. Парольный login здесь отключён. В частной установке этот режим по умолчанию выключен.

| Путь | Назначение |
|---|---|
| `GET /api/public/config` | Анонимные `enabled`, `session_ttl_seconds`, `ai_enabled`; включённый флаг AI не доказывает успешный вызов модели |
| `POST /api/public/session` | Разрешённый Origin, `{role: "employee" или "hr"}` → `SessionResponse`; создаёт/использует личную учебную копию |
| `GET /api/public/examples/employees.json` | Три собственных вымышленных профиля для проверки импорта в публичный каталог |
| `GET /api/public/examples/activity_history.csv` | Соответствующая вымышленная история; загружается вместе с JSON через обычный preview/commit |

Выбор роли относится только к собственной демонстрации, не к рабочей базе. HttpOnly-cookie `cq_workspace` выбирает отдельный SQLite-файл по сохранённому хешу случайного токена; `cq_session` и CSRF защищают обычные endpoints. Смена роли отзывает предыдущие сессии посетителя. Данные другой копии недоступны даже при одинаковом employee_id. Истечение TTL возвращает к новой копии; бесплатный хост может удалить временный диск раньше. Выход остаётся доступен после исчерпания квот.

Ограничения публичного сервера: 32 активные копии, до 6 новых в минуту, тело запроса до 256000 байт, до 100 изменений/10 импортных запросов за сессию; AI — до 10 запросов на копию и 60 в час на сервер. Preview и commit импорта учитываются отдельно. При превышении — 413/429 с обычным ErrorResponse, без подмены результата. В частном режиме остаются исходные ограничения импортера; эти публичные квоты не включаются. Секрет модели задаётся только в окружении сервера.
