...
Исправление оценки SLA: исходное ТЗ, строки 54/164/277, допускает UI до 2 секунд и AI до **10 секунд**. Наблюдение 5033,10 ms укладывается в AI-требование; прежнее сравнение с 5 секундами было ошибкой отчёта. Одно наблюдение не подтверждает стабильность и p95. Три quality cases не заменяют скрытую приёмку. Русские backend-шаблоны пока не локализованы; старые сохранённые карточки получают новые тексты только после повторной генерации. Полная HR-аналитика пробелов/участия остаётся незавершённой по документации frontend; текущий прогон не добавляет её.
